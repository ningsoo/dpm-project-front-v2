...
function MypagePageContent() {
  ...
  if (!user) {
    if (!initialized || loading) {
      const skeletonPopSub = (() => { const p = searchParams.get('popSubTab'); return (p === 'usage' || p === 'purchase') ? p : 'purchase'; })();
      const skeletonDonSub = (() => { const d = searchParams.get('donationSubTab'); return (d === 'sent' || d === 'received') ? d : 'sent'; })();
      const skeletonStSub = (() => { const s = searchParams.get('settlementSubTab'); return (s === 'history' || s === 'register' || s === 'request') ? s : 'history'; })();
      if (tabParam) {
        return (
          <MypageSkeleton
            activeTab={getValidTab(tabParam)}
            tabs={TABS}
            dateInputMax={getTodayDateString()}
            dateInputDefaultStart={getDate30DaysAgo()}
            popSubTab={skeletonPopSub}
            donationSubTab={skeletonDonSub}
            settlementSubTab={skeletonStSub}
          />
        );
      }
  ...
  return (
    <div className={`${styles.wrap} ${pageReady ? styles.wrapVisible : styles.wrapHidden}`}>
      <ProfileCard
        ...
        email={user.email}
        phoneDisplay={formatPhone11(user.phoneNumber) || '—'}
        popBalance={user.popBalance ?? 0}
        receivedLikes={receivedLikes}
        passwordless={user?.passwordless === true}
        pwlsRegisterLoading={pwlsRegisterLoading}
        pwlsWithdrawalLoading={pwlsWithdrawalLoading}
        onPencilClick={() => {
        ...
        onPwlsWithdraw={() => setShowPwlsWithdrawalModal(true)}
        onPwlsRegister={(email: string) => openPwlsRegister(email)}
      ...
      <MypageTabs
        ...
        onTabChange={(tabId) => {
          setTab(tabId);
          router.replace(`/mypage?tab=${tabId}`);
        }}
        tabs={TABS}
      />
      ...
      <div className={`${styles.content} ${tabVisible ? styles.contentVisible : styles.contentHidden}`}>
        ...
        {displayedTab === 'reports' && (
          <ReportsSection
            dateInputMax={getTodayDateString()}
            initialDateRange={{ start: getDate30DaysAgo(), end: getTodayDateString() }}
          />
        )}
